...
plan ──→ plan-check ──→ write(×3) ──→ validate ──→ review ──→ fix* ──→ open-pr
 ...
 LLM       LLM(ゲート)     LLM          非LLM(ゲート) LLM       LLM       非LLM
                                    ...
                                    validate-repair*
                                     修復・1回だけ
                                     LLM → 非LLMで再検証 ──→ 通れば review へ
```

`fix` は review が pass しなかったときだけ走る。
`validate-repair` は **validate が落ちたときだけ**走る（2026-09-01 追加）。
...
| `validate`（非LLM） | seedバリデータ・型・lint・テスト | **`validate-repair` が1回だけ修復を試みる。**直れば同じ非LLM検証を通し直して review へ進む。直らなければPRが出ず、Issueが立つ |

### `validate-repair`（2026-09-01 追加）

**1件のNGでその夜の6本×4言語が全損するのを止めるため。** 実績＝2026-08-31（必須表現1件で6本全損）・2026-08-24・2026-08-22×3。

`plan-check` には「9本作って6本採る」という予備があるが、`validate` には無かった。ここが唯一、機械が落としたものを拾い直す場所。

- **`validate` 自体はLLMを使わないまま。** 修復は別ジョブに分け、修復後は**同じ非LLM検証をもう一度通してから**先へ送る
- **修復は1回だけ。2周目は無い**（直らなければ失敗させて人間に回す）
- 修復役に最初に判断させるのは「**記事が悪いのか、検査の指定が悪いのか**」。2026-08-31 の実例＝本文は「2020年（令和2年）4月1日」と西暦・和暦を併記して正しく書けていたのに、`REQUIRED_PHRASES` が「令和2年4月1日」の連続一致を求めていて落ちた。このときの正解は**検査の指定を本文の表記に合わせる**ことで、本文から括弧を外すことではない
- **NG判定そのものを削ってゲートを緩めるのは禁止**。裏取りできていない条文番号・日付を書き足して通すのも禁止
- `notify-failure` / `close-failure-issues` は `failure()` / `success()` を使わない条件に変えてある。どちらも祖先ジョブの結果を拾うため、**修復に成功した夜まで「失敗」と数えてしまう**
