...
あわせてキーワード一致を2段階にした。タイトル・カテゴリ・slug を先に見て、
そこで決まらないときだけタグを見る。全文を一度に見ていたため、タグに偶然入っていた語が
主題を上書きしていた（「就業規則 10人の義務」がタグの「ハラスメント」で相談窓口の絵になる等）。
タイトルに小見出しを列挙するハブ記事は `SLUG_OVERRIDES` で明示する。
...
3. **`THEME_RULES` にルールを追加する**（狭いテーマほど前に置く。
   キーワードは広すぎないこと。「認証」のような語は別文脈の記事を巻き込む）
