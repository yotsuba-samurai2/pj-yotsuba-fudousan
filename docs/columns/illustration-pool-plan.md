...
- **新規34点の生成は浦松側（GPT Image 2）で行う。**
  生成プロンプト全文・原画ファイル名・最終ファイル名は `docs/columns/illustration-prompts.md` に作成済み。
  生成工程はこの環境の外にあるため、リポジトリ側は原画の納品を受けてから変換・配置・マニフェスト作成を行う。

### 未承認（第3段に進む前に判断が必要）

| # | 論点 | 現状 |
|---|---|---|
| 2 | 52テーマのこの内訳でよいか（realestate 18 / legal 17 / labor 17） | プロンプト集はこの内訳を前提に作成済み。変更があればプロンプト集も差し替える |
| 3 | 配置先を `public/hero/` に揃える逸脱を認めるか | プロンプト集は `public/hero/` 前提でファイル名を記載。変更はファイル名の付け替えのみで済む |
| 4 | 文字を画像に重ねない構成でよいか | 未着手 |
| 5 | 既存18点＋フォールバックだけで**先行実装**してよいか | 未着手。これが承認されれば画像納品を待たず「挿絵0枚」を解消できる |
| 6 | `ColumnForm` への `ogImage` 入力欄追加を範囲に含めるか | 未着手 |
| 7 | 台湾draft 9件（トップレベルが繁体中文）の扱い | 未着手 |
| 8 | `labor-joseikin-16x9.webp` の天秤を理由に差し替えるか | 差し替える場合は新規35点になる（`illustration-prompts.md` §5） |
