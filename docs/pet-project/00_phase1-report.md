...
4. **既存学区ページに、指示書に反する既存表示が2点ある**。① 空表示「現在、この学区で**ご紹介できる**賃貸物件はありません」（4言語）は第8.2章の「断定しない」に反する。② 比較表の相談枠「この一覧の掲載件数の**平均2倍以上**の物件をご提案できます」（**4言語すべて**。当初「ja・zh-tw・zh」と書いたが英語版にも “at least twice” があった。2026-09-24 訂正）は、根拠のない数値表現の禁止（第13章）に抵触するおそれがある。②は浦松の判断 D-4＝削除により、Phase 2 で4言語とも削除した（`20_phase2.md`）。①は Phase 5 で直す。
...
| 根拠のない数値表現をしない | 相談枠「この一覧の掲載件数の**平均2倍以上**の物件をご提案できます」（`RentalComparison.tsx:21,27,33,39`。4言語すべて） | **D-4＝削除（Phase 2 で実施済み）** |
...
| D-4 | 学区ページ「平均2倍以上」の表示 | 根拠資料あり→出典を明記して維持／無し→削除 | 根拠を確認のうえ判断 |
| D-5 | Phase 2 の範囲 | 保存分離＋許諾台帳＋集計関数（統計は無効）まで／LP を先に | 指示書どおり Phase 2 から。ただし**統計が当面無効なら Phase 3（住宅LP・受付）を先行**させても効果は同じ |

**判断の結果（浦松・2026-09-24）**：D-1 受付テーブルを新設（まずペットの3フォーム・Phase 3）／D-2 ペット調査は学区と別の保存先／D-3 推奨どおり（`/services`＋メニュー）／D-4 削除（Phase 2 で4言語とも実施）／D-5 指示書どおり Phase 2 から。Phase 2 の実装内容は `20_phase2.md`。
