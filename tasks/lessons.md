...
## 2026-09-09 コラムmutation APIとキャッシュ失効

- 公開状態・locale・slug・事業変更の失効はmutation APIで完結させる。管理画面の追加HTTPリクエストに依存するとsoft deleteやupsertが漏れる。
- 更新前のbusiness/slugを保持し、新旧両URLと一覧・ページ送り・言語索引を失効する。
- Next.js 16.2.10のrevalidatePath/revalidateTagは予約API。HTTP成功前にストレージの失効完了を確認し、非同期失敗も500で返す。内部executor依存は隔離し、Next更新時は実productionサーバーで失効障害まで再検証する。
- モックの呼び出し確認だけでなく、SSG/ISRの200と404を先にキャッシュし、mutation後の最初のGETとヘッダー言語リンクを検証する。
