...
if (process.env.COLUMN_TEST_PAGINATION === '1') {
  const ids = [];
  const { columns } = await api('GET', '?business=labor');
  const count = columns.filter(column => column.status === 'published' && (!column.locales?.length || column.locales.includes('en'))).length;
  const nextPage = Math.max(2, Math.ceil(count / 20) + 1);
  const additions = (nextPage - 1) * 20 + 1 - count;
  const url = `/en/labor/column/page/${nextPage}`;
  await check(url, 404);
  try {
    ...
    await check(url, 200);
    await check(url, 200);
    await api('PATCH', `/${ids.at(-1)}`, {status:'deleted'});
    await check(url, 404);
  } finally {
...
if (process.env.COLUMN_TEST_FAILURE_FILE) {
  ...
  try {
    const failure = await api('PATCH',`/${id}`,{status:'draft'},503);
    assert.equal(failure.mutationSucceeded, true);
    assert.match(failure.error, /保存・削除は完了/);
    ...
    results.push({failure:'asynchronous cache storage rejection',status:503,dbStatus:saved.column.status});
