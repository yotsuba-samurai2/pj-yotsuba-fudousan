...
export function fixture(): RentalImport & { reins: NonNullable<RentalImport["reins"]>; photoPermission: NonNullable<RentalImport["photoPermission"]> } {
  ...
  const v: RentalImport & { reins: NonNullable<RentalImport["reins"]>; photoPermission: NonNullable<RentalImport["photoPermission"]> } = {
    ...
    reins: { propertyId: "test-100", building: "検証用マンション", address: "東京都文京区検証町1-2-3", unit: "001", advertising: "allowed", evidence, rent: { yen: 85500, evidence: { ...evidence, quote: "賃料8.55万円", reference: "https://system.reins.jp/" } }, availability: "available", listingEvidence: { ...evidence, reference: "https://system.reins.jp/", quote: "掲載中・test-100・001号室", authenticated: true, siteOperational: true, exactRoomMatched: true } },
