...
describe("他社を含む候補物件ごとの掲載数", () => {
  ...
  it("取得元が募集中でも同一号室の掲載終了があれば除外", () => { const c = check(); c.listings[0].status = "ended"; const result = validateRentalImport({ ...fixture(), portalChecks: [c] }, NOW); expect(result).toMatchObject({ ok: false, reasons: ["掲載終了のため登録対象外です: suumo / a"] }); });
  it("別号室の掲載終了では対象を除外しない", () => { const c = check(); c.listings[0].status = "ended"; c.listings[0].match = "different"; expect(validateRentalImport({ ...fixture(), portalChecks: [c] }, NOW).ok).toBe(true); });
