...
describe("独立レビューの回帰検証", () => {
  ...
  it("条件を選択した後の本文・翻訳の照合を省略できず、照合後の変更も検出", () => {
    ...
    const evidence = (quote: string) => ({ checkedAt: NOW.toISOString(), reference: "匿名募集資料", quote });
    v.conditionChoices = [conditionChoiceSchema.parse({ rule: "strictest", field: "guarantor", basis: "同じ保証会社・同じ算定基準", options: [
      { value: "初回50%", burden: { initialPercent: 50 }, evidence: evidence("初回50%") },
      { value: "初回100%", burden: { initialPercent: 100 }, evidence: evidence("初回100%") },
    ] })];
