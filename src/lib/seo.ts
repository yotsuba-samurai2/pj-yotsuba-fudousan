 ...
 *
 * 【監査原則】sameAs は「同一エンティティの別ページ」を宣言する場所であり、裏取りできたものだけを出す。
 ...
 * 別事業体を同一視する出力は業法分離と真っ向から矛盾する。空でも必ず明示的に持たせ、
 * 暗黙のフォールバックで他事業体の識別子を借りない（labor-entity.test.ts が検査）。
 */
export const LABOR_SAME_AS = [
  "https://www.wikidata.org/wiki/Q141252134",
  ...(GBP_URL.labor ? [GBP_URL.labor] : []),
] as const;
