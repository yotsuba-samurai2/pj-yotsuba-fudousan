...
export const COMPLIANCE_VALUE_PATCHES: { from: string; to: string }[] = [
  // ─────────────────────────────────────────────────────────────
  // 2026-08-09 追加：翻訳DBの groupBusinesses の事業体名（M-7）
  //
  // 「法人」は事実に反する。浦松が2026年9月1日に開設するのは
  // **個人の社会保険労務士事務所**であり、社会保険労務士法人ではない。
  //
  // 現在は sr-strip.ts が name を見て配列要素ごと除去しているためHTMLに出ない。
  // しかし 9/1 に SR_LAUNCHED=true で除去が止まると、そのまま可視化する。
  // → **開業前に直しておく必要がある。**
  //
  // 是正後の「事務所」名も SR_ENTITY_NAME_RE に一致するため、
  // 開業までの除去は引き続き効く（2026-08-09 確認済み）。
  //
  // ★ 完全一致で照合する。不一致のパッチはドライランで報告されるので、
  //   DBに実際に何が入っているかはドライランの結果で確定させる。
  // ─────────────────────────────────────────────────────────────
  { from: "四葉社會保險勞務士法人", to: "四葉社會保險勞務士事務所" },
  { from: "四叶社会保险劳务士法人", to: "四叶社会保险劳务士事务所" },
  { from: "四葉社会保険労務士法人", to: "四葉社会保険労務士事務所" },
  { from: "四葉社會保險勞務士法人（2026年9月開業預定）", to: "四葉社會保險勞務士事務所（2026年9月開業預定）" },
  { from: "四叶社会保险劳务士法人（2026年9月开业预定）", to: "四叶社会保险劳务士事务所（2026年9月开业预定）" },
  // ─────────────────────────────────────────────────────────────
  ...
  },
  {
    ...
    to: "株式会社・合同会社の設立手続きをサポート。定款作成から申請書類までを支援します。事務所探しは四葉不動産が別契約で受任し、登記は司法書士が対応します。",
  },
...
export const COMPLIANCE_SCAN_TERMS: string[] = [
  // 2026-08-06 追加：U12（書面での業務提携の有無）は「書面はない」で確定（浦松回答）。
  // したがって「提携◯◯士」と書けない。「◯◯士におつなぎします／ご紹介します」にとどめる。
  "提携税理士",
  "提携司法書士",
  "提携弁護士",
  "提携社会保険労務士",
  "提携の社会保険労務士",
  // ★ 日本語だけを見ると必ず取りこぼす（2026-08-09 に実際に取りこぼした）。
  //   繁体字「合作」・簡体字「合作」・英語「partner/affiliated」を必ず併記する。
  //   同種の失敗＝PR #178（社労士エントリ除去が繁体字を見ていなかった）。
  "合作稅理士",
  "合作税理士",
  "合作司法書士",
  "合作司法书士",
  ...
  // 2026-08-09 追加：事業体名の「法人」（開設するのは個人事務所であり法人ではない）
  "社會保險勞務士法人",
  "社会保险劳务士法人",
  "社会保険労務士法人",
  "ワンストップ",
...
/**
 * **条件付きで可**。2026-07-29 石井弁護士確認（U6解決）。
 *
 * 相談の窓口が同じであることと、受任・契約・請求が分離していることは両立する。
 * ただし**同じページ内に分離受任の明示を併記する**ことが条件（正本 6-3）。
 * スキャンは条件の充足を判定できないため、**違反ではなく参考として別枠で表示する**。
 *
 * ここに入れずに COMPLIANCE_SCAN_TERMS へ残すと、許容済みの表現が毎回警告として出て、
 * 本物の違反が埋もれる（2026-08-10 に130件中約100件が誤検出だった）。
 */
export const COMPLIANCE_SCAN_TERMS_CONDITIONAL: string[] = [
  "一つの窓口",
  "同じ窓口",
  "窓口は一つ",
  "窓口を一本化",
...
/**
 * **個別に許容済み**（path の末尾一致 × 語）。
 * ここに載せたものはスキャン結果から除く。**理由と決裁日を必ず書く。**
 */
export const COMPLIANCE_SCAN_ALLOWLIST: Array<{
  path: string;
  term: string;
  reason: string;
  decidedAt: string;
  ...
  {
    path: "legal.homePage.faq.2.question",
    term: "まとめて",
    reason: "「不動産と法務をまとめて相談できますか？」＝相談の可否を問う設問。受任の一体性を述べていない",
    decidedAt: "2026-08-10 浦松",
  },
  {
    path: "legal.homePage.services.4.description",
    term: "連携して対応",
    reason: "「不動産相続は四葉不動産と連携して対応します」",
    decidedAt: "2026-08-10 浦松",
  },
  {
    path: "legal.footerDescription2",
    term: "まとめて",
    reason: "「ビジネスと暮らしの手続きをまとめてサポートします」",
    decidedAt: "2026-08-10 浦松",
  },
  {
    path: "realestate.home.whyYotsuba.strengths.4.description",
    term: "チームで対応",
    reason: "「各分野の専門家と連携。複雑な案件もチームで対応します」",
    decidedAt: "2026-08-10 浦松",
  },
];
...
export const COMPLIANCE_SCAN_EXCLUSIONS: string[] = [
  ...
  "東京開業ワンストップセンター",
  "TOSBEC",
  // 2026-08-10 追加：コラムの内部リンク文。一体提供とは無関係で約30件の誤検出源だった。
  // 「詳しくは〜にまとめています」の形。「まとめて契約」等の違反とは意味が違う。
  "にまとめています",
  "でまとめています",
  "にまとめました",
  "をまとめています",
  "まとめています",
  // 2026-08-10 追加：制度用語。障害者グループホームのサテライト型は
  // 「本体の共同生活住居と一体的に運営／支援する」と法令・通知で説明される。
  "一体的に運営",
  "一体的に支援",
  // 2026-08-10 追加：賃貸借の一括借り上げ。士業の業際とは無関係
  "一括してお借り",
  "一括して借り",
];
...
export const COMPLIANCE_COLUMN_PATCHES: ColumnPatch[] = [
  ...
  // ─────────────────────────────────────────────────────────────
  // 2026-08-06 追加：M-4「提携◯◯士」の是正（U12 解決＝書面での業務提携はない）
  //
  // 浦松回答（2026-08-06）により U12 が確定した。書面での提携がない以上、
  // 「提携税理士」等と書くと確認できない関係を断定することになる。
  // 「◯◯士におつなぎします／ご紹介します」にとどめる（正本 6-4-5）。
  // あわせて、他事務所が「受任します／お引き受けします」と読める形にしない（6-4-6）。
  //
  // ★ 適用は配列順。長い語句を先に置くこと。
  //    先に "提携税理士" を処理すると "提携税理士がお引き受けします" が拾えなくなる。
  // ─────────────────────────────────────────────────────────────
  {
    slug: "*",
    from: "給与課税など税務の判断は提携税理士がお引き受けします",
    to: "給与課税など税務の判断は税理士におつなぎします",
    note: "M-4：他事務所の受任を当方が約束する形にしない（6-4-6）",
  },
  {
    slug: "*",
    from: "提携弁護士と連携して進めます",
    to: "弁護士におつなぎします",
    note: "M-4：「連携」は業務の一体提供を示唆しうる。「おつなぎ」に統一",
  },
  {
    slug: "*",
    from: "提携弁護士と連携します",
  ...
  { slug: "*", from: "提携税理士", to: "税理士", note: "M-4：U12＝書面での提携なし" },
  { slug: "*", from: "提携司法書士", to: "司法書士", note: "M-4：同上" },
  { slug: "*", from: "提携弁護士", to: "弁護士", note: "M-4：同上" },
  { slug: "*", from: "提携社会保険労務士", to: "社会保険労務士", note: "M-4：同上" },
  // 中国語版
  { slug: "*", from: "合作稅理士", to: "稅理士", note: "M-4：繁体字" },
  { slug: "*", from: "合作司法書士", to: "司法書士", note: "M-4：繁体字" },
  { slug: "*", from: "合作律師", to: "律師", note: "M-4：繁体字" },
  { slug: "*", from: "合作税理士", to: "税理士", note: "M-4：簡体字" },
  { slug: "*", from: "合作司法书士", to: "司法书士", note: "M-4：簡体字" },
  { slug: "*", from: "合作律师", to: "律师", note: "M-4：簡体字" },
  // 英語版
  { slug: "*", from: "partner tax accountant", to: "a tax accountant", note: "M-4：英語" },
  { slug: "*", from: "partner judicial scrivener", to: "a judicial scrivener", note: "M-4：英語" },
  { slug: "*", from: "affiliated tax accountant", to: "a tax accountant", note: "M-4：英語" },
];
