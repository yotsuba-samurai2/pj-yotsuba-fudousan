...
const rental = (brokerFee?: "full" | "half" | "free") => ({ dealType: "rental" as const, spec: { dealType: "rental" as const, brokerFee } as never });
...
describe("仲介手数料 無料・半額の表示（ATBB→athome ルール v1.1 第5節）", () => {
  ...
  it("絞り込みは半額・無料の賃貸だけ", () => {
    expect([rental("free"), rental("half"), rental("full"), rental()].filter(isDiscountedBrokerFee)).toHaveLength(2);
  ...
  it("値引きの理由（業者間の事情）を表示文言に含めない", () => {
    for (const locale of ["ja", "en", "zh-tw", "zh"] as const) {
      for (const fee of ["full", "half", "free"] as const) expect(brokerFeeLine(rental(fee), locale)).not.toMatch(/\bAD\b|広告料|広告費/);
    }
  });
  it("スキーマは満額・半額・無料だけを受け付ける", () => {
