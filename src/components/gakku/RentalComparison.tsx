...
export function RentalComparison({ rows, locale }: { rows: PublicRentalSummary[]; locale: LangCode }) {
  ...
  const expiry = rows.map(r => r.expiresAt).sort((a, b) => Date.parse(a) - Date.parse(b))[0];
  ...
  return <RentalInquiryProvider locale={locale}><section className="mt-8" aria-label={c.title}>
    ...
    <div className="mt-4 space-y-4">
      {rows.map(r => {
        ...
        return <article id={r.id} key={r.id} className="rounded-xl border border-border bg-surface p-4 sm:p-5">
          ...
          <p className="mt-3 text-xs text-text-muted">{c.checked}：{date(r.checkedAt)} / {c.until}：{date(r.expiresAt)}</p>
    ...
    <PropertyLegalBlock property={{ infoUpdatedAt: date(latest), nextUpdateAt: date(expiry) }} locale={locale} />
