import { saleSchoolDistrict, schoolSalePath, SCHOOL_SALE_COPY } from "@/lib/sale-school-district";
...
export function PropertySchoolDistrict({ property, locale }: { property: PublicProperty; locale: LangCode }) {
  ...
  return <section aria-label={c.label} className="mt-5 rounded-xl border-2 border-primary/25 bg-primary-tint p-4 sm:p-5">
    ...
    {district.status === "determined" ? <Link href={addLocalePrefix(property.dealType === "rental" ? schoolRentalPath(district.school.slug) : schoolSalePath(district.school.slug), locale)} className="mt-3 inline-flex rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90">{property.dealType === "rental" ? c.view : SCHOOL_SALE_COPY[locale].view} →</Link>
