...
export function SchoolRentalIndex({ properties, locale }: { properties: PublicProperty[]; locale: LangCode }) {
      ...
      <ul className="mt-6 grid gap-3 sm:grid-cols-2">
        {listSchools().map(school => <li key={school.slug}>
          <Link href={addLocalePrefix(schoolRentalPath(school.slug), locale)} className="flex h-full items-center justify-between gap-3 rounded-xl border border-border bg-surface p-5 transition-colors hover:border-primary">
            ...
            <span className="shrink-0 rounded-full bg-primary-tint px-3 py-1 text-sm font-semibold text-primary">{c.count.replace("{count}", String(groups.get(school.slug)?.length ?? 0))} →</span>
          </Link>
        </li>)}
      </ul>
...
export function SchoolRentalListings({ school, properties, locale }: { school: SchoolInfo; properties: PublicProperty[]; locale: LangCode }) {
  const c = SCHOOL_RENTAL_COPY[locale];
  ...
  return <>
    ...
    <article className="mx-auto max-w-3xl px-4 pb-16">
      <header className="rounded-2xl border border-primary/20 bg-primary-tint p-5 sm:p-8">
        ...
        <p className="mt-3 text-lg font-semibold text-primary">{c.count.replace("{count}", String(listings.length))}</p>
        <DistrictSourceNote locale={locale} />
      </header>
      {listings.length ? <ul className="mt-6 space-y-3">{listings.map(p => <li key={p.slug}><PropertyCard p={p} locale={locale} /></li>)}</ul>
        : <p className="mt-6 rounded-xl border border-border p-6 leading-relaxed text-text">{c.empty}</p>}
