...
// （Android Chrome はPDFを外部アプリへ渡す、LINE等のアプリ内ブラウザは全画面表示で戻る操作が見つからない、
//   iOS Safari はPDF表示中にツールバーが隠れる）、リンク先をサイト内ビューア /sample/[kind] に変えた。
// PR #382 の「同じタブで開いてブラウザの戻るで帰る」方式では解消しなかった。PDFはビューアからダウンロードできる。
...
export type { PropertySearchSampleKind } from "@/lib/property-search-samples";

/** ビューアの「戻る」が履歴を戻ってよいと判断できるよう、いまのページを覚えておく */
function rememberReturnPage() {
  try {
    window.sessionStorage.setItem(SAMPLE_RETURN_KEY, window.location.pathname + window.location.hash);
  } catch {
    // プライベートブラウズ等で sessionStorage が使えない場合は、ビューア側が既定の戻り先を使う
  }
}

export function PropertySearchSampleTeaser({ kind, locale = "ja", page }: { kind: PropertySearchSampleKind; locale?: LangCode; page: string }) {
  const t = SAMPLE_COPY[kind][locale];
  const asset = SAMPLE_ASSETS[kind][locale];
  return (
    <section aria-label={t.tag} className="my-10 grid overflow-hidden rounded-2xl border border-primary/20 bg-primary-tint md:grid-cols-[1fr_230px]">
      <div className="p-6 sm:p-8">
        ...
        <Link
          href={addLocalePrefix(sampleViewerPath(kind), locale)}
          onClick={() => {
            rememberReturnPage();
            gaEvent("property_search_sample_pdf_open", { page, location: "teaser", kind, locale, viewer: "html" });
          }}
          className="mt-5 inline-flex min-h-12 items-center rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-white hover:bg-primary-dark focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
        >
  ...
  return (
    <section id="property-search" aria-labelledby="property-search-heading" className="scroll-mt-28 space-y-7">
      ...
      <div className="grid gap-6 rounded-2xl border border-primary/20 bg-primary-tint p-5 sm:p-7 md:grid-cols-[200px_1fr]">
        <Image src={SAMPLE_ASSETS.welfare.ja.preview} alt="架空の候補8件の賃料・初期費用・アクセスを整理した一覧表" width={707} height={1000} sizes="200px" className="mx-auto h-auto w-48 border border-border bg-white shadow-md" />
        <div className="self-center">
          ...
          <Link
            href={sampleViewerPath("welfare")}
            onClick={() => {
              rememberReturnPage();
              gaEvent("property_search_sample_pdf_open", { page: "/nagare", location: "property-search", kind: "welfare", locale: "ja", viewer: "html" });
            }}
            className="mt-5 inline-flex min-h-12 items-center rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-white hover:bg-primary-dark focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
          >
            通所系福祉施設版を見る（9ページ）<span className="ml-2" aria-hidden="true">→</span>
          </Link>
          <p className="mt-2 text-xs text-text-muted">このサイトの中で表示します。画面上部の「元のページに戻る」でこのページへ戻れます。PDFの保存・印刷もできます。</p>
        </div>
      </div>
