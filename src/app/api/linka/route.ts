...
const LANG_NAMES: Record<string, string> = {
  en: "English（英語）",
  // K-2d：本番実測で zh-tw 指定でも簡体字が返る事象を確認 → 字種を強く明示する
  "zh-tw": "繁體中文（台灣で使う正體字。簡体字は1文字も使わないこと）",
  zh: "简体中文（中国大陸で使う簡体字。繁体字は1文字も使わないこと）",
};
...
- **指定された字種を厳守する**（繁體中文＝正體字のみ／简体中文＝簡体字のみ。混在させない）。
- **Markdown記法を使わない**（**強調**・見出し・箇条書き記号・番号付きリストの装飾を書かない）。
  表示側はプレーンテキストで描画するため、記号がそのまま画面に出てしまう。普通の文章として訳す。
- 出力はJSONのみ。前置き・説明・コードフェンスを書かない。`;

/**
 * K-2d｜画面表示用のサニタイズ（決定論的な保険）。
 * AIがMarkdown装飾（**強調** など）を混ぜることがあり、結果カードはプレーンテキスト描画のため
 * 記号が生のまま画面に出てしまう（本番実測）。プロンプトで禁止したうえで、機械的にも除去する。
 * ※内容は変えない（装飾記号を落とすだけ）。
 */
...
/** K-2d｜画面に出る自由文からMarkdown装飾を落とす（日本語・翻訳後の両方に適用）。url/id/typeは触らない。 */
function sanitizeResult(r: LinkaResult): LinkaResult {
  const out = { ...r } as LinkaResult;
  if ("message" in out && typeof out.message === "string" && out.message) {
    (out as { message?: string }).message = stripMarkdown(out.message);
  }
  if (out.type === "concierge") {
    if (out.kento?.length) out.kento = out.kento.map((k) => stripMarkdown(k));
    if (out.services?.length) {
      out.services = out.services.map((s) => ({
        ...s,
        reason: s.reason ? stripMarkdown(s.reason) : s.reason,
      }));
    }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (locale !== "ja") {
      ...
      // K-2d：Markdown装飾を落としてから返す（結果カードはプレーンテキスト描画のため）
      if (translated) return NextResponse.json(sanitizeResult(translated));
    }
    return NextResponse.json(sanitizeResult(resolved));
  } catch {
