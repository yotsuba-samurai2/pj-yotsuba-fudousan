...
import { REALESTATE_COLUMNS_P6_SEED } from "@/lib/data/realestate-columns-p6-seed";
...
/**
 * 不動産コラム第6弾（1本）のupsert投入。
 *
 * 台湾企業の日本拠点づくり×オフィス・住まいの実務（ja/en/zh-tw/zh 4言語）。
 * business=realestate。宅建業法3条・35条、経営・管理の令和7年10月16日施行改正、
 * Tokyo Hubの所在地・実績は2026-08-20に一次確認済み。
 * slug基準の冪等upsert。再実行しても重複しない。
 */
export default function SeedRealestateP6Page() {
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState<ItemResult[]>(
    REALESTATE_COLUMNS_P6_SEED.map((a) => ({
      slug: a.slug,
      title: a.title,
      status: "pending",
    })),
  ...
  const handleRun = async () => {
    setRunning(true);
    for (let i = 0; i < REALESTATE_COLUMNS_P6_SEED.length; i++) {
      const article = REALESTATE_COLUMNS_P6_SEED[i];
      try {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-1 text-lg font-bold">不動産コラム第6弾（1本）投入</h1>
      <p className="mb-4 text-sm text-text-muted">
        台湾企業の日本拠点づくり×オフィス・住まいの実務。business=realestate／status=published／
        locales=[&quot;ja&quot;,&quot;en&quot;,&quot;zh-tw&quot;,&quot;zh&quot;]の4言語で投入します。
      </p>
