...
import { REALESTATE_COLUMNS_SEED } from "@/lib/data/realestate-columns-seed";
...
/**
 * 不動産コラム（3本）のバルクupsert投入。
 *
 * scripts/realestate-columns/*.md から
 * `npx tsx scripts/seed-realestate-columns.ts --emit-ts` で焼き込んだ
 * src/lib/data/realestate-columns-seed.ts を、ブラウザの管理者セッション経由で
 * slug基準の冪等upsertで投入する（seed-gh / seed-office と同型・再実行しても重複しない）。
 * business=realestate（/column）／locales=["ja"]／status="published"。
 */
export default function SeedRealestatePage() {
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState<ItemResult[]>(
    REALESTATE_COLUMNS_SEED.map((a) => ({
      slug: a.slug,
      title: a.title,
      status: "pending",
    })),
  ...
  const handleRun = async () => {
    setRunning(true);
    for (let i = 0; i < REALESTATE_COLUMNS_SEED.length; i++) {
      const article = REALESTATE_COLUMNS_SEED[i];
      try {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-1 text-lg font-bold">不動産コラム（3本）バルク投入</h1>
      <p className="mb-4 text-sm text-text-muted">
        business=realestate／locales=[&quot;ja&quot;]／status=published でupsert投入します（slug基準・冪等）。
        再実行しても重複しません。原稿の正本＝scripts/realestate-columns/*.md。
      </p>
