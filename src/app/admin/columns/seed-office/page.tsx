...
import { OFFICE_COLUMNS_SEED } from "@/lib/data/office-columns-seed";
 ...
 */
export default function SeedOfficePage() {
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState<ItemResult[]>(
    OFFICE_COLUMNS_SEED.map((a) => ({
      slug: a.slug,
      title: a.title,
      status: "pending",
    })),
  ...
  const handleRun = async () => {
    setRunning(true);
    for (let i = 0; i < OFFICE_COLUMNS_SEED.length; i++) {
      const article = OFFICE_COLUMNS_SEED[i];
      try {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-1 text-lg font-bold">オフィス許認可コラム（3本）バルク投入</h1>
      <p className="mb-4 text-sm text-text-muted">
        business=legal／locales=[&quot;ja&quot;]／status=published でupsert投入します（slug基準・冪等）。
        再実行しても重複しません。原稿の正本＝scripts/office-columns/*.md（検収済みドラフト準拠）。
      </p>
