...
import { GH_COLUMNS_SEED } from "@/lib/data/gh-columns-seed";
...
/**
 * GHコラムクラスタP1（5本）のバルクupsert投入。
 *
 * scripts/gh-columns/*.md（浦松検収済み 2026-07-23）から
 * `npx tsx scripts/seed-gh-columns.ts --emit-ts` で焼き込んだ
 * src/lib/data/gh-columns-seed.ts を、ブラウザの管理者セッション経由で
 * slug基準の冪等upsertで投入する（seed-taiwan と同型・再実行しても重複しない）。
 * status="published"（検収済みのため公開状態で投入。business=legal / locales=["ja"]）。
 */
export default function SeedGhPage() {
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState<ItemResult[]>(
    GH_COLUMNS_SEED.map((a) => ({
      slug: a.slug,
      title: a.title,
      status: "pending",
    })),
  ...
  const handleRun = async () => {
    setRunning(true);
    for (let i = 0; i < GH_COLUMNS_SEED.length; i++) {
      const article = GH_COLUMNS_SEED[i];
      try {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-1 text-lg font-bold">GH開設コラムP1（5本）バルク投入</h1>
