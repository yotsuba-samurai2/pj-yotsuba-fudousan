...
export async function generateColumnMetadata(page: number): Promise<Metadata> {
  ...
  const availableLocales = await getColumnPageLocales("realestate", page);
  ...
  return buildPageMetadata({
    ...
    path: columnPagePath("/column", page),
    keywords: [
      "不動産 コラム",
      "部屋探し コツ",
      "敷金 礼金",
...
export async function renderColumnList(page: number) {
  const locale = await getRequestLocale();
  const result = await getColumnPage("realestate", locale, page);
  ...
  return (
    <>
      ...
      <ColumnListPageContent columns={columns} page={page} total={result.total} totalPages={result.totalPages} />
