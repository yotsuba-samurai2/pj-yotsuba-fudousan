...
const COPY: Partial<Record<LangCode, Copy>> = {
  ja: {
    ...
    s3: {
      ...
      p2: "ただし、適用には建物・期間・売却条件などの要件があり、使えるかどうかの判断は税務の専門領域です。適用要件の判断は税理士にご確認ください。当社は税理士のご紹介と、売却そのもののサポートを承ります。",
    },
    s4: {
    ...
    s6: {
      ...
      closingPre:
        "相続登記など法務手続きに関わる書類作成は、併設の四葉行政書士事務所が別契約で受任します。登記申請の代理は司法書士の業務のため、司法書士をご紹介します。海外在住の方のご相談にも対応しています。中国語圏にお住まいの方は、",
  ...
  "zh-tw": {
    ...
    s3: {
      ...
      p2: "不過，適用上有建物、期間、出售條件等要件，能否使用的判斷屬於稅務的專門領域。適用要件的判斷請向稅理士（日本的稅務專業資格）確認。本公司承接稅理士的介紹，以及出售本身的支援。",
    },
    s4: {
      h2: "條件困難的空屋也歡迎諮詢",
    ...
    faq: [
      ...
      },
      {
        ...
        a: "針對繼承空屋的讓渡所得，設有3,000萬日圓特別扣除的制度。不過適用上有要件，能否使用的判斷屬於稅務的專門領域，請向稅理士（日本的稅務專業資格）確認。本公司承接稅理士的介紹，以及出售本身的支援。",
  ...
  zh: {
    ...
    s3: {
      ...
      p2: "不过，适用上有建筑物、期间、出售条件等要件，能否使用的判断属于税务的专门领域。适用要件的判断请向税理士（日本的税务专业资格）确认。本公司承接税理士的介绍，以及出售本身的支援。",
    },
    s4: {
      h2: "条件困难的空屋也欢迎咨询",
    ...
    faq: [
      ...
      },
      {
        ...
        a: "针对继承空屋的转让所得，设有3,000万日元特别扣除的制度。不过适用上有要件，能否使用的判断属于税务的专门领域，请向税理士（日本的税务专业资格）确认。本公司承接税理士的介绍，以及出售本身的支援。",
