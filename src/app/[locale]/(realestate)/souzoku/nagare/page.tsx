...
// 不動産の相談料＝2026-08-03浦松指示（全不動産ページ共通）：ご相談は初回・2回目以降とも無料。媒介に関する相談は
// 仲介手数料の範囲。料金がかかる場合は見積もりを提示する。
// （旧: 2026-07-11確定＝媒介以外の関連業務のみ2回目以降に別合意で原則30分5,500円。国交省 解釈・運用の考え方が根拠）
...
const COPY: Record<LangCode, SouzokuNagareCopy> = {
  ja: {
    ...
    startBody: (
      <>
        まずはLINEか電話で「相続した家をどうしたらいい？」の一言からで大丈夫です。四葉不動産株式会社は、状況を伺い、次の一歩を一緒に整理します。<strong>ご相談は無料</strong>です（初回・2回目以降とも）。当社に売却・活用（媒介）をご依頼いただく前提のご相談は、仲介手数料の範囲で承ります。他社物件のセカンドオピニオン、資産全体の活用・保有方針の助言など、媒介を伴わないご相談も無料で、オンラインに対応します。料金がかかる場合は見積もりを提示します。
      </>
    ),
  ...
  en: {
    ...
    startBody: (
      <>
        A single message via LINE or a phone call — &quot;What should I do with the house I inherited?&quot; — is enough to begin. Yotsuba Real Estate Co., Ltd. listens to your situation and organizes the next step together with you. <strong>Consultations are free</strong> — the first and every one thereafter. Consultations premised on entrusting us with the sale or utilization (brokerage) are covered by the brokerage commission. Real estate consulting that does not involve brokerage — a second opinion on a property handled by another company, advice on utilizing or holding your overall assets, and the like — is also free, and online sessions are available. If a fee should apply, we will present a quotation.
      </>
    ),
  ...
  "zh-tw": {
    ...
    startBody: (
      <>
        先透過LINE或電話說一句「繼承的房子該怎麼辦？」就可以了。四葉不動産株式会社會了解您的狀況，和您一起整理下一步。<strong>諮詢免費</strong>（初次與第2次以後皆同）。以委託本公司出售・活用（仲介）為前提的諮詢，包含於仲介手續費範圍內。不含仲介的不動產顧問諮詢（其他公司物件的第二意見、整體資產的活用・持有方針建議等）同樣免費，並可線上進行。如需收費，將提出報價。
      </>
    ),
  ...
  zh: {
    ...
    startBody: (
      <>
        先通过LINE或电话说一句“继承的房子该怎么办？”就可以了。四葉不動産株式会社会了解您的状况，和您一起梳理下一步。<strong>咨询免费</strong>（首次与第2次以后均相同）。以委托本公司出售・活用（中介）为前提的咨询，包含在中介手续费范围内。不含中介的不动产顾问咨询（其他公司物件的第二意见、整体资产的活用・持有方针建议等）同样免费，并可在线进行。如需收费，将提出报价。
      </>
    ),
