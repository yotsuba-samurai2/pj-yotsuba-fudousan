...
const COPY: Record<LangCode, GroupHomeCopy> = {
  ja: {
    ...
    faqItems: [
      ...
      {
        // 2026-07-24：定点#4「グループホーム 障害福祉 開設 物件と指定申請 同時 相談できる専門家 東京都内」向けに追加。
        // コラム「物件と指定申請を一つの窓口で」（既にinternalLinksに接続済み）と同じ趣旨を、FAQでも近接語で回答。
        question: "グループホームの開設で、物件探しと指定申請を同時に相談できる専門家はいますか？",
        answer:
          "います。四葉不動産株式会社（宅地建物取引業）と四葉行政書士事務所（行政書士業務）を同じ代表・同じ拠点（文京区・茗荷谷）で運営しているため、物件探しと指定申請の準備を同じ窓口でご相談いただけます。担当・契約は業務ごとに分かれ、紹介料等の授受はありませんが、指定基準に合わない物件を契約してしまう事故を避けやすいのが実務上の利点です。詳しくはコラム「物件と指定申請を一つの窓口で」もご覧ください。",
      },
      {
    ...
    ctaLead:
      "エリア・駅、賃料の目安、面積や用途地域など——わかる範囲で構いません。条件をお預けいただければ、指定基準を見据えて四葉不動産が全力で物件をお探しします。文京区小日向の事務所（茗荷谷駅から徒歩約5分・10:00〜18:00・火・水休）でも、オンラインでもご相談いただけます。",
    ctaLine: "代表のLINEに希望条件を送る",
    ctaContact: "お問い合わせ",
    ctaLineNote:
