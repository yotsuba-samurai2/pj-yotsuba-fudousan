...
const COPY: Partial<Record<LangCode, Copy>> = {
  ...
  zh: {
    ...
    s4: {
      ...
      p1: "在华人圈的继承手续中，常需要将中国大陆的户口簿、公证处的公证书，或台湾的户籍谨本、除户谨本等当地文件，整理成可用于日本手续的形式。",
  ...
  return (
    ...
    >
      ...
      {/* 対応できないこと＝B-4共通コンポーネント（浦松確定文言・一字一句変更なし）。
          日本語版のみ描画する（CannotHandle.tsx の方針どおり呼び出し側で locale を判定）。
          2026-07-19 C-6-1：中国語版では非表示とする（浦松判断）。 */}
      {locale === "ja" && <CannotHandle bare />}
