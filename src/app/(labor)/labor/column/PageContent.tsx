    ...
    <>
      <section className="relative grid min-h-[40vh] place-content-center overflow-hidden border-b border-border pt-24 pb-16 sm:pt-32 sm:pb-32 md:pt-40 md:pb-40">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h1 className="mt-3 text-3xl font-bold sm:text-4xl md:text-5xl">{t("labor.columnPage.title")}</h1>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-text-muted sm:text-base">{t("labor.columnPage.heroDescription1")}<br />{t("labor.columnPage.heroDescription2")}</p>
        </div>
      </section>
      ...
      <section className="py-14 sm:py-20 md:py-28">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="space-y-4 sm:space-y-6">
            {columns.map((col) => (
              <Link key={col.slug} href={`/labor/column/${col.slug}`} className="group block overflow-hidden rounded-xl border border-border bg-surface p-4 transition-all duration-300 hover:border-primary/30 hover:shadow-lg sm:p-6 md:p-8">
          ...
          <div className="mt-12 flex items-center justify-center gap-2">
            <button disabled className="flex h-10 w-10 items-center justify-center rounded-lg border border-border text-text-muted transition-colors disabled:opacity-30" aria-label={t("common.prevPage")}><ChevronLeft size={18} /></button>
            <span className="gradient-line flex h-10 w-10 items-center justify-center rounded-lg text-sm font-bold text-white">1</span>
            <button disabled className="flex h-10 w-10 items-center justify-center rounded-lg border border-border text-text-muted transition-colors disabled:opacity-30" aria-label={t("common.nextPage")}><ChevronRight size={18} /></button>
          </div>
        </div>
      </section>
    </>
