...
const FAQS: { q: string; a: string; link?: { href: string; label: string } }[] = [
  ...
  {
    // 2026-08-14 追加（浦松指示）：freeeサポートとの関係＝相互補完。
    // ★freee側の限界（「freeeは個別事例に答えられない」等）としては書かず、役割の分担として書く
    //（他社サービスの能力への断定は、公表資料を一次確認しない限り書かない）。
    // 27条の独占は手続の作成・提出代行（1号〜2号）。個別判断（3号相当）は独占ではないため
    // 「独占業務」と書く対象を手続に限定している。
